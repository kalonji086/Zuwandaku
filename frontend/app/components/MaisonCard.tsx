...
import { MapPin, BedDouble, Bath, Maximize2, Home, Tag, KeyRound, Car, Eye, Phone } from "lucide-react";
...
const STATUS: Record<string,{label:string;color:string}> = { AVAILABLE:{label:"Disponible",color:SL.green}, RENTED:{label:"Louée",color:SL.yellow}, SOLD:{label:"Vendue",color:SL.red}, RESERVED:{label:"Réservée",color:"#8b5cf6"} };

function Placeholder() {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-3 relative overflow-hidden"
      style={{ background:`linear-gradient(135deg,${SL.bgDeep} 0%,#0b1a2e 100%)` }}>
      <div className="absolute inset-0 opacity-[0.04]"
        style={{ backgroundImage:`linear-gradient(${SL.blue} 1px,transparent 1px),linear-gradient(90deg,${SL.blue} 1px,transparent 1px)`, backgroundSize:"32px 32px" }} />
      <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-32 h-16 rounded-full blur-2xl opacity-20" style={{ background:SL.blue }} />
      <div className="relative z-10 flex flex-col items-center gap-2">
        <div className="relative">
          <div className="w-14 h-14 rounded-2xl flex items-center justify-center"
            style={{ background:`${SL.blue}18`, border:`1px solid ${SL.blue}35`, boxShadow:`0 0 20px ${SL.blue}25` }}>
          ...
          <div className="absolute -bottom-1 -right-1 w-6 h-6 rounded-lg flex items-center justify-center"
            style={{ background:SL.blue, boxShadow:`0 0 8px ${SL.blue}70` }}>
            <Home size={12} className="text-white" />
          </div>
        </div>
        <p className="text-xs font-bold tracking-widest uppercase" style={{ color:SL.blue2 }}>ZUWAndaku</p>
        <p className="text-xs" style={{ color:SL.muted }}>Maison</p>
      </div>
    </div>
  );
}

export default function MaisonCard({ p }: { p: any }) {
  ...
  return (
    <>
      <div className="group flex flex-col overflow-hidden rounded-2xl border transition-all duration-300 hover:-translate-y-1.5 cursor-pointer"
        ...
        onMouseEnter={e=>{ e.currentTarget.style.borderColor=SL.blue; e.currentTarget.style.boxShadow=`0 8px 32px ${SL.blue}20`; }}
        ...
        <div className="relative h-48 overflow-hidden" style={{ background:SL.bgDeep }} onClick={() => setShowView(true)}>
          {p.photos?.[0] ? (
            <>
              <Image src={p.photos[0]} alt={p.description||"Maison"} width={400} height={300}
          ...
          <span className="absolute top-3 right-3 flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold"
            style={{ background:`${SL.blue}25`, border:`1px solid ${SL.blue}50`, color:"#fff", backdropFilter:"blur(6px)" }}>
        ...
        <div className="flex flex-col flex-1 p-4 gap-2.5">
          ...
          <h3 className="font-semibold text-sm line-clamp-2 leading-snug" style={{ color:SL.text }}>
            {p.description || `Maison à ${p.commune||"louer"}`}
          </h3>
          <div className="flex flex-wrap gap-1.5">
            {p.chambres    && <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-md" style={{ background:`${SL.blue}12`, color:SL.blue2, border:`1px solid ${SL.blue}25` }}><BedDouble size={10} />{p.chambres} ch.</span>}
            ...
            {p.parking     && <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-md" style={{ background:"rgba(245,158,11,0.1)", color:"#fbbf24", border:"1px solid rgba(245,158,11,0.25)" }}><Car size={10} />Parking</span>}
          </div>
          <div className="mt-auto pt-2.5 border-t" style={{ borderColor:SL.border }}>
            <div className="flex items-center justify-between mb-2.5">
              <div>
                <p className="text-lg font-extrabold leading-none" style={{ color:SL.blue2 }}>${p.price?.toLocaleString("fr-FR")??"—"}</p>
            ...
            <div className="flex gap-2">
              <button onClick={() => setShowView(true)}
                ...
                style={{ background:`${SL.blue}15`, color:SL.blue2, border:`1px solid ${SL.blue}30` }}
                onMouseEnter={e=>{ e.currentTarget.style.background=`${SL.blue}25`; }}
                onMouseLeave={e=>{ e.currentTarget.style.background=`${SL.blue}15`; }}>
                <Eye size={13} />Voir
              </button>
