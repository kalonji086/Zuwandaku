...
export default function DeleteVehicleModal({ isOpen, onClose, vehicle, onConfirm, isLoading }: Props) {
      ...
      <div className="bg-gray-950 border border-white/10 rounded-2xl w-full max-w-md">
        ...
        <div className="border-b border-white/10 px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-base font-bold text-white">Supprimer le véhicule</h2>
          </div>
        ...
        <div className="px-6 py-6">
          <div className="flex items-start gap-4 bg-red-500/5 border border-red-500/20 rounded-xl p-4 mb-6">
            ...
            <div>
              ...
              <p className="text-white/50 text-xs">Le véhicule et toutes ses données seront définitivement supprimés.</p>
            </div>
          </div>

          <div className="flex items-center gap-4 bg-white/5 border border-white/10 rounded-xl p-4">
            <img
              src={vehicle.photos?.[0] || 'https://placehold.co/60x60?text=V'}
              alt={vehicle.marque}
              className="w-14 h-14 object-cover rounded-lg border border-white/10 shrink-0"
            />
            <div className="min-w-0">
              <p className="text-white font-semibold text-sm">{vehicle.marque} {vehicle.modele}</p>
              <p className="text-white/40 text-xs mt-0.5">{vehicle.type} — {vehicle.annee}</p>
              <p className="text-white font-bold text-sm mt-1">
                {vehicle.priceSale ? `$${vehicle.priceSale?.toLocaleString()}` : `$${vehicle.pricePerDay}/jour`}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="border-t border-white/10 px-6 py-4 flex gap-3 justify-end">
          ...
          <button onClick={() => onConfirm(vehicle.id)} disabled={isLoading}
