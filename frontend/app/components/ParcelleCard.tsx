...
import { MapPin, Maximize2, Landmark, Tag, FileCheck, Compass, TreePine, Eye, Phone } from "lucide-react";
...
const STATUS: Record<string,{label:string;color:string}> = { AVAILABLE:{label:"Disponible",color:SL.green}, RENTED:{label:"Occupée",color:SL.yellow}, SOLD:{label:"Vendue",color:SL.red}, RESERVED:{label:"Réservée",color:"#8b5cf6"} };
const ACCENT = "#10b981";

function Placeholder() {
  return (
    <div className="w-full h-full flex flex-col items-center justify-center gap-2 relative overflow-hidden"
      style={{ background:`linear-gradient(135deg,${SL.bgDeep} 0%,#071a12 100%)` }}>
      <div className="absolute inset-0 opacity-[0.04]"
        style={{ backgroundImage:`linear-gradient(${ACCENT} 1px,transparent 1px),linear-gradient(90deg,${ACCENT} 1px,transparent 1px)`, backgroundSize:"36px 36px" }} />
      {["top-3 left-3","top-3 right-3","bottom-3 left-3","bottom-3 right-3"].map((pos,i)=>(
        <div key={i} className={`absolute ${pos} w-3 h-3 rounded-sm opacity-25`} style={{ border:`2px solid ${ACCENT}` }} />
      ))}
      ...
      <div className="relative z-10 flex flex-col items-center gap-2">
        ...
        <p className="text-xs" style={{ color:SL.muted }}>Parcelle</p>
      </div>
    </div>
  );
}

export default function ParcelleCard({ p }: { p: any }) {
    ...
    <>
      <div className="group flex flex-col overflow-hidden rounded-2xl border transition-all duration-300 hover:-translate-y-1.5 cursor-pointer"
        ...
        <div className="relative h-48 overflow-hidden" style={{ background:SL.bgDeep }} onClick={() => setShowView(true)}>
          {p.photos?.[0] ? (
            <>
              <Image src={p.photos[0]} alt={p.description||"Parcelle"} width={400} height={300}
        ...
        <div className="flex flex-col flex-1 p-4 gap-2.5">
          ...
          <h3 className="font-semibold text-sm line-clamp-2 leading-snug" style={{ color:SL.text }}>
            {p.description || `Parcelle à ${p.commune||"vendre"}`}
          </h3>
          <div className="flex flex-wrap gap-1.5">
            {p.surface      && <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-md" style={{ background:`${ACCENT}12`, color:ACCENT, border:`1px solid ${ACCENT}25` }}><Maximize2 size={10} />{p.surface} m²</span>}
            {p.titreFoncier && <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-md" style={{ background:"rgba(37,99,235,0.1)", color:"#60a5fa", border:"1px solid rgba(37,99,235,0.25)" }}><FileCheck size={10} />Titre foncier</span>}
            {p.viabilisee   && <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-md" style={{ background:`${ACCENT}12`, color:ACCENT, border:`1px solid ${ACCENT}25` }}><TreePine size={10} />Viabilisée</span>}
            {p.orientation  && <span className="flex items-center gap-1 text-xs px-2 py-0.5 rounded-md" style={{ background:"rgba(245,158,11,0.1)", color:"#fbbf24", border:"1px solid rgba(245,158,11,0.25)" }}><Compass size={10} />{p.orientation}</span>}
          </div>
          <div className="mt-auto pt-2.5 border-t" style={{ borderColor:SL.border }}>
            <p className="text-lg font-extrabold mb-2.5" style={{ color:ACCENT }}>${p.price?.toLocaleString("fr-FR")??"—"}</p>
            <div className="flex gap-2">
              ...
              <button onClick={() => setShowContact(true)}
                className="flex-1 flex items-center justify-center gap-1.5 py-2 rounded-lg text-xs font-semibold transition-all"
                style={{ background:"rgba(37,99,235,0.1)", color:"#60a5fa", border:"1px solid rgba(37,99,235,0.3)" }}
