...
import {
  LayoutDashboard, Home, Car, FileText, Plus, Settings, TrendingUp,
  MapPin, Building2, ExternalLink, ToggleLeft, ToggleRight, CheckCircle, XCircle
} from 'lucide-react';
import ModuleGuard from '../../components/ModuleGuard';

type Module = {
  id: string; label: string; description: string; href: string;
  icon: React.ElementType; color: string; groupe: string; actif: boolean;
};

const INIT: Module[] = [
  // Propriétaire
  { id:'p-dashboard',   label:'Dashboard Propriétaire', description:'Vue d\'ensemble, KPIs, revenus, activité récente.',           href:'/proprietaire',              icon:LayoutDashboard, color:'#1a6dff', groupe:'Espace Propriétaire', actif:true  },
  { id:'p-analytics',   label:'Statistiques',           description:'Graphiques de performance, taux d\'occupation, tendances.',   href:'/proprietaire/analytics',    icon:TrendingUp,      color:'#1a6dff', groupe:'Espace Propriétaire', actif:true  },
  { id:'p-properties',  label:'Mes Propriétés',         description:'Liste et gestion des biens immobiliers publiés.',             href:'/proprietaire/properties',   icon:Home,            color:'#1a6dff', groupe:'Espace Propriétaire', actif:true  },
  { id:'p-vehicles',    label:'Mes Véhicules',          description:'Gestion des véhicules associés au compte propriétaire.',      href:'/proprietaire/vehicles',     icon:Car,             color:'#1a6dff', groupe:'Espace Propriétaire', actif:true  },
  { id:'p-add',         label:'Ajouter un bien',        description:'Formulaire de publication d\'un nouveau bien ou véhicule.',   href:'/proprietaire/add-property', icon:Plus,            color:'#1a6dff', groupe:'Espace Propriétaire', actif:true  },
  { id:'p-contracts',   label:'Contrats',               description:'Suivi des contrats de location et de vente.',                 href:'/proprietaire/contracts',    icon:FileText,        color:'#1a6dff', groupe:'Espace Propriétaire', actif:true  },
  { id:'p-settings',    label:'Paramètres',             description:'Paramètres du compte propriétaire.',                         href:'/proprietaire/settings',     icon:Settings,        color:'#1a6dff', groupe:'Espace Propriétaire', actif:true  },
  // Admin immobilier
  { id:'a-appartements',label:'Gestion Appartements',   description:'Administration des appartements, validation, modération.',   href:'/admin/gestion-appartements',icon:Building2,       color:'#7b61ff', groupe:'Admin Immobilier',    actif:true  },
  { id:'a-parcelles',   label:'Gestion Parcelles',      description:'Administration des parcelles, titres fonciers.',             href:'/admin/gestion-parcelles',   icon:MapPin,          color:'#7b61ff', groupe:'Admin Immobilier',    actif:true  },
  { id:'a-proprietes',  label:'Propriétés Admin',       description:'Vue admin de toutes les propriétés de la plateforme.',       href:'/admin/proprietes',          icon:Home,            color:'#7b61ff', groupe:'Admin Immobilier',    actif:true  },
  { id:'a-rental',      label:'Locations',              description:'Suivi des biens en location active.',                        href:'/admin/rental-properties',   icon:FileText,        color:'#7b61ff', groupe:'Admin Immobilier',    actif:false },
];

const GROUPE_COLOR: Record<string,string> = { 'Espace Propriétaire':'#1a6dff', 'Admin Immobilier':'#7b61ff' };
...
export default function ImmobilierModule() {
  const [modules, setModules] = useState<Module[]>(INIT);
  const [toast, setToast] = useState('');

  const showToast = (m:string) => { setToast(m); setTimeout(()=>setToast(''),2500); };
  const toggle = (id:string) => setModules(prev=>prev.map(m=>{
    if(m.id!==id) return m;
    const next=!m.actif; showToast(`${m.label} ${next?'activé':'désactivé'}`); return {...m,actif:next};
  }));

  const groupes = ['Espace Propriétaire','Admin Immobilier'];
  const actifs = modules.filter(m=>m.actif).length;

  return (
    <ModuleGuard role="DEPT_ADMIN" module="Properties">
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
        {[{label:'Total modules',value:modules.length,color:'#1a6dff',icon:Home},{label:'Actifs',value:actifs,color:'#00e5a0',icon:CheckCircle},{label:'Désactivés',value:modules.length-actifs,color:'#f87171',icon:XCircle}].map(s=>(
