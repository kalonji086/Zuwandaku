...
import {
  ...
  ExternalLink, ToggleLeft, ToggleRight, CheckCircle, XCircle, UserCheck,
} from 'lucide-react';
...
const INIT: Module[] = [
  { id: 'p-dashboard',  label: 'Dashboard',        description: "Vue d'ensemble, KPIs, revenus et activité récente.",          href: '/proprietaire',              icon: LayoutDashboard, color: '#1a6dff', groupe: 'Espace Propriétaire', actif: true  },
  ...
  { id: 'a-espaces',    label: 'Espace Admin',      description: 'Vue admin de l\'espace propriétaire (gestion & contrôle).',  href: '/admin/espaces/proprietaire',icon: UserCheck,       color: '#00c2ff', groupe: 'Admin Propriétaire',  actif: true  },
];

const GROUPE_COLOR: Record<string, string> = { 'Espace Propriétaire': '#1a6dff', 'Admin Propriétaire': '#00c2ff' };
...
export default function ProprietaireModule() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 12 }}>
        {[{ label: 'Total modules', value: modules.length, color: '#1a6dff', icon: UserCheck }, { label: 'Actifs', value: actifs, color: '#00e5a0', icon: CheckCircle }, { label: 'Désactivés', value: modules.length - actifs, color: '#f87171', icon: XCircle }].map(s => (
      ...
      {['Espace Propriétaire', 'Admin Propriétaire'].map(groupe => {
