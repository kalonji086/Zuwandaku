...
import {
  LayoutDashboard, Car, FileText, Settings, TrendingUp, Plus,
  ExternalLink, ToggleLeft, ToggleRight, CheckCircle, XCircle, Wrench
} from 'lucide-react';
...
const INIT: Module[] = [
  ...
  { id:'pv-dashboard', label:'Dashboard Propriétaire',  description:'Vue d\'ensemble des véhicules, revenus, locations en cours.',  href:'/proprietaire',           icon:LayoutDashboard, color:'#00e5a0', groupe:'Espace Propriétaire', actif:true  },
  { id:'pv-vehicles',  label:'Mes Véhicules',           description:'Liste, édition et gestion des véhicules publiés.',             href:'/proprietaire/vehicles',  icon:Car,             color:'#00e5a0', groupe:'Espace Propriétaire', actif:true  },
  { id:'pv-contracts', label:'Contrats Location',       description:'Contrats de location véhicules, suivi et historique.',         href:'/proprietaire/contracts', icon:FileText,        color:'#00e5a0', groupe:'Espace Propriétaire', actif:true  },
  { id:'pv-analytics', label:'Statistiques',            description:'Performance des véhicules, taux d\'utilisation, revenus.',     href:'/proprietaire/analytics', icon:TrendingUp,      color:'#00e5a0', groupe:'Espace Propriétaire', actif:true  },
  { id:'pv-add',       label:'Ajouter véhicule',        description:'Formulaire d\'ajout d\'un nouveau véhicule à la flotte.',      href:'/proprietaire/add-property',icon:Plus,          color:'#00e5a0', groupe:'Espace Propriétaire', actif:true  },
  { id:'pv-settings',  label:'Paramètres',              description:'Paramètres du compte propriétaire véhicules.',                 href:'/proprietaire/settings',  icon:Settings,        color:'#00e5a0', groupe:'Espace Propriétaire', actif:true  },
  // Admin véhicules
  { id:'av-gestion',   label:'Gestion Véhicules Admin', description:'Administration de tous les véhicules de la plateforme.',       href:'/admin/gestion-vehicules', icon:Car,            color:'#ffcc00', groupe:'Admin Véhicules',     actif:true  },
  { id:'av-vehicules', label:'Véhicules Admin',         description:'Vue admin complète, validation, modération des annonces.',     href:'/admin/vehicules',         icon:Wrench,         color:'#ffcc00', groupe:'Admin Véhicules',     actif:true  },
];

const GROUPE_COLOR: Record<string,string> = { 'Espace Propriétaire':'#00e5a0', 'Admin Véhicules':'#ffcc00' };
...
export default function VehiculesModule() {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      {toast&&<div style={{ position:'fixed', top:20, right:20, zIndex:200, background:'#00e5a0', color:'#000', padding:'10px 18px', borderRadius:10, fontSize:13, fontWeight:600 }}>{toast}</div>}
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
        {[{label:'Total modules',value:modules.length,color:'#00e5a0',icon:Car},{label:'Actifs',value:actifs,color:'#00e5a0',icon:CheckCircle},{label:'Désactivés',value:modules.length-actifs,color:'#f87171',icon:XCircle}].map(s=>(
      ...
      {['Espace Propriétaire','Admin Véhicules'].map(groupe=>{
