...
import {
  UtensilsCrossed, ChefHat, ShoppingCart, ClipboardList, Settings,
  LayoutDashboard, ExternalLink, ToggleLeft, ToggleRight, CheckCircle, XCircle, Star
} from 'lucide-react';
...
const INIT: Module[] = [
  // Hôtel restaurant
  { id:'hr-dashboard',  label:'Dashboard Hôtel',       description:'Dashboard principal de l\'hôtel avec section restaurant intégrée.', href:'/hotel',             icon:LayoutDashboard, color:'#ff6b35', groupe:'Hôtel Restaurant', actif:true  },
  { id:'hr-chef',       label:'Chef Cuisine',          description:'Interface du chef : commandes reçues, plats, gestion cuisine.',      href:'/hotel/chef-cuisine', icon:ChefHat,         color:'#ff6b35', groupe:'Hôtel Restaurant', actif:true  },
  { id:'hr-taches',     label:'Tâches Cuisine',        description:'Tâches assignées à l\'équipe cuisine, suivi en temps réel.',        href:'/hotel/taches',      icon:ClipboardList,   color:'#ff6b35', groupe:'Hôtel Restaurant', actif:true  },
  { id:'hr-billing',    label:'Facturation Repas',     description:'Facturation des repas servis aux clients de l\'hôtel.',             href:'/hotel/billing',     icon:ShoppingCart,    color:'#ff6b35', groupe:'Hôtel Restaurant', actif:true  },
  { id:'hr-reports',    label:'Rapports Restaurant',   description:'Rapports de ventes, plats populaires, revenus restaurant.',         href:'/hotel/reports',     icon:Star,            color:'#ff6b35', groupe:'Hôtel Restaurant', actif:false },
  { id:'hr-parametres', label:'Paramètres Restaurant', description:'Configuration du menu, horaires, politique de service.',            href:'/hotel/parametres',  icon:Settings,        color:'#ff6b35', groupe:'Hôtel Restaurant', actif:true  },
  // Client restaurant
  { id:'cr-restaurant', label:'Restaurant Client',     description:'Interface client pour commander depuis l\'espace client.',          href:'/client/restaurant', icon:UtensilsCrossed, color:'#ffcc00', groupe:'Espace Client',    actif:true  },
  { id:'cr-hotel',      label:'Hôtel Client',          description:'Accès hôtel et restaurant depuis l\'espace client.',               href:'/client/hotel',      icon:LayoutDashboard, color:'#ffcc00', groupe:'Espace Client',    actif:true  },
];

const GROUPE_COLOR: Record<string,string> = { 'Hôtel Restaurant':'#ff6b35', 'Espace Client':'#ffcc00' };
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:12 }}>
        {[{label:'Total modules',value:modules.length,color:'#ff6b35',icon:UtensilsCrossed},{label:'Actifs',value:actifs,color:'#00e5a0',icon:CheckCircle},{label:'Désactivés',value:modules.length-actifs,color:'#f87171',icon:XCircle}].map(s=>(
      ...
      {['Hôtel Restaurant','Espace Client'].map(groupe=>{
